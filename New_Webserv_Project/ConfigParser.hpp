...
/*   Updated: 2021/06/09 11:48:29 by timvancitte   ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
{
	public:
		// typedef Server::Servercon		t_v_server_conf;
		typedef ServerConfiguration::directives		directives;
		typedef ServerBlock		ServerBlock;
		typedef Server::serverBlocks	serverBlocks;
		typedef Server::allServers		allServers;
		typedef Server::ipPort			ipPort;
	...
	static	void	convertTokens(configTokens &tokens, allServers &_allServers) {
		...
		while (!tokens.empty()) {
			conf = tokens.front();
			duplicate = false;
			std::string &listen = conf._directives["listen"];
			std::string	const & server_name = conf._directives["server_name"];
			duplicate = false;

			formatIp(listen);
			//add check for invalid ip:port
			serverBlocks	&serverBlocks = _allServers[listen];
			if (v_server_blocks.size() > 0) {
				for (ServerBlock::const_iterator _server = v_server_blocks.begin(); v_server != v_server_blocks.end(); ++v_server) {
